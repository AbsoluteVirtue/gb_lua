...
function intel:PostRun()
	...
	if gamemode.HasGameOption("opforcount") then
		self.OpForCount = tonumber(gamemode.GetGameOption("opforcount"))
	elseif gamemode.HasGameOption("terroristcount") then
		self.OpForCount = tonumber(gamemode.GetGameOption("terroristcount"))
	end

	-- Ensures we cannot spawn more AI then this map can handle.
	self.MaxOpforCount = #AllSpawns
	self.OpForCount = math.max(self.OpForCount, 1)
	...
	local Difficulty = 1
	if gamemode.HasGameOption("Difficulty") then
		Difficulty = tonumber(gamemode.GetGameOption("Difficulty"))
	end
	gamemode.AddGameSetting("difficulty", 0, 4, 1, Difficulty);
...
function intel:OnProcessCommand(Command, Params)
	if Command == "terroristcount" or Command == "opforcount" then
		if Params ~= nil then
